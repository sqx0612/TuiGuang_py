...
#提交request获得jsfuck模块
def rq_get_jf(html,header_get,proxy):
    resp=requests.get(url=html,headers=header_get,timeout=5,proxies=proxy)
...
#换ip模块
def ip():
     html_ip='http://api.xdaili.cn/xdaili-api//greatRecharge/getGreatIp?spiderId=94b4cb54a1ee4c53b9e5637f6a85f0d5&orderno=YZ202291331597klpRL&returnType=1&count=1'
     ip_post=requests.get(url=html_ip).text
     proxies = {
   "http": "http://"+ip_post
 }
     return proxies
...
#返回验证码模块
def send_code(cookie,code_url,code,proxies):
    header_sendcode = {
    ...
    code_sent_respone = requests.post(url=code_send_html, headers=header_sendcode, data=data_code,proxies=proxies)
